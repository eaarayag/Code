...
def get_scope(partition, test_type=''):
    ...
    `uio_a_0`). Every other partition is classified as Partition Level.
    `test_type` is accepted for symmetry with the SCAN pipeline but is not
    used here because TAP has no `<stack>_retarget_*` tests.
    """
